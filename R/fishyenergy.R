...
bem_grow <- function(start_M2 = 100, temperature, parms.intrinsic, parms.temporal, C_eq, R_eq, match.table)
{
  ...
  dframe.sim_parms$WT_mean <- dframe.sim_parms$WT_mean / 10
...
#' @export
bem_sensitive <- function(start_M2 = 100, temperature, parms.intrinsic, parms.temporal, C_eq, R_eq, parm.dev)
{
  # set parm deviation
  ...
  for(i in 1:length(list.parms_sens)){
    for(j in 1:nrow(list.parms_sens[[i]])){
      df.output[j,i] <- bem_grow(start_M2,
                                 temperature,
                                 parms.intrinsic = list.parms_sens[[i]][j,],
                                 parms.temporal,
                                 C_eq,
                                 R_eq)[nrow(temperature),"M2_cum"]
    }
  }
