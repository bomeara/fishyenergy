...
### Vignette objectives
- Learn how to validate a bioenergetics model. That is, you will simulate 365 days of growth and then use empirical length-at-age estimates from a field study to identify consumption (CP) and activity (ACT) values that produce realistic growth.
...
We developed the **bem_validate** function to address this challenge using an alternative approach. What the function does is identify pairs of CP and ACT values that simulate realistic growth. Inputs of bem_validate include a subset of those you're familiar with from the **bem_grow** function including temperature, parms.intrinsic, parms.temporal, C_eq, and R_eq.
...
3) The number of CP and ACT parameter sets to test in the growth validation. CP values are drawn from a uniform probability distribution with a minimum of 0.0 and a maximum of 1.0. ACT values are also drawn from a uniform distribution with a minimum and maximum of 1.0 and 4.0, respectively.
```{r}
